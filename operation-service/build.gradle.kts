...
dependencies {
    ...
    // 게시판 본문 저장 시 HTML 화이트리스트 정화
    implementation("org.jsoup:jsoup:1.18.3")
    // Mail (jakarta.mail) — notification 슬라이스의 SMTP EmailChannel (폴리글랏 흡수).
    // 자격증명(MAIL_USERNAME/PASSWORD) 미설정 시 채널이 스스로 비활성이라 그대로 뜬다.
    ...
    // education 슬라이스 테스트가 H2(MODE=PostgreSQL) 인메모리로 부팅한다 (board·operation 은 Testcontainers)
