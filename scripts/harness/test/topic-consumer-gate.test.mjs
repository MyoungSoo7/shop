...
const PUBLISH_ONLY = new Map([
  ...
  // expirynotice — 소멸 예고. 위 항목들과 사유의 *종류*가 다르다. 앞의 것들은 소비자가 경계
  // 밖(정산·GL)에 있어 안 붙는 것이고, 이쪽은 붙을 자리가 이 저장소 안인데 아직 없는 것이다.
  // 이 구분을 안 적으면 2026-08-28 의 실패가 반복된다 — 그때 여섯 토픽이 "소비자는 저장소 밖"
  // 이라고 적혀 있었지만 사실은 소비자가 아직 안 생겼을 뿐이었고, 생기자 여섯이 한꺼번에 빠졌다.
  ['lemuel.expirynotice.upcoming',
    '발행 전용 — 소멸 예고. 소비자가 경계 밖이 아니라 알림 슬라이스가 이 저장소에 아직 없다. 생기면 ADR 0024 로 편입'],
]);
