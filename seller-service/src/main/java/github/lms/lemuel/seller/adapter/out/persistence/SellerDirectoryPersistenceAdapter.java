...
/** 조직·구성원 프로젝션의 적재와, 로그인 사용자 → {@link SellerScope} 해석. */
@Slf4j
@Component
@RequiredArgsConstructor
class SellerDirectoryPersistenceAdapter implements LoadSellerScopePort, SellerDirectoryProjectionPort {

    private final SellerOrganizationJpaRepository organizationRepository;
    private final SellerMemberJpaRepository memberRepository;

    @Override
    public Optional<SellerScope> findByUserId(long userId) {
        ...
        if (rows.size() > 1) {
            ...
            // 이유는, 이 상태에서 셀러가 올리는 상품이 "자기가 기대한 조직" 의 것이 아닐 수 있어서다.
    ...
    @Override
    public List<SellerMemberView> findActiveMembers(long organizationId) {
        return memberRepository.findActiveMemberRows(organizationId).stream()
                .map(row -> new SellerMemberView(
                        Rows.longAt(row, 0),
                        Rows.longAt(row, 1),
    ...
    public void upsertOrganization(long organizationId, String name, OrgType type, String externalRef,
        ...
        organizationRepository.upsert(organizationId, name, type.name(), externalRef, sellerId, ownerUserId);
    }
    ...
    private static SellerScope toScope(Object[] row) {
