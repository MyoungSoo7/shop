...
/** {@code seller_members} 적재 + 로그인 사용자 → 소속 조직 해석. */
interface SellerMemberJpaRepository extends JpaRepository<SellerMemberJpaEntity, Long> {

    @Modifying
    @Query(value = """
            INSERT INTO seller.seller_members
     ...
     * {@code uq_seller_members_active} 다 — 같은 사람이 나갔다 다시 들어오면 membership_id 가
     * 다른 새 행이 오는데, 옛 행을 남겨 두어도 status 로 걸러지므로 충돌하지 않는다.
     */
    @Modifying
    @Query(value = """
            UPDATE seller.seller_members
     ...
     *
     * <p>여기서는 그 선택이 파트너 콘솔보다 무겁다 — 파트너는 남의 매출을 <i>보는</i> 것이
     * 잘못될 뿐이지만, 셀러는 <b>남의 이름으로 상품을 올리게</b> 된다. 그래서 순서를 고정하고,
     * 둘 이상이면 어댑터가 경고를 남긴다.
     *
     * <p>반환 순서: organization_id, name, org_type, seller_id, role
     */
    @Query(value = """
            SELECT m.organization_id, o.name, o.org_type, o.seller_id, m.role
              FROM seller.seller_members m
              JOIN seller.seller_organizations o ON o.organization_id = m.organization_id
