 ...
 * 증적이 아니다.
 *
 * <p><b>탭이 여럿인 이유</b>: 감사 테이블은 서비스마다 자기 DB 에 따로 있다(MSA 경계). 커머스
 * 조작과 자금 조작과 운영 조작은 애초에 다른 테이블이며, 하나로 합쳐 보여 주려면 서비스 간
 * 조인이 필요해 경계를 깨야 한다. 그래서 화면이 "어느 쪽을 보는가"를 명시적으로 고르게 한다.
 * 탭을 늘리는 것이 합치는 것보다 정직하다 — 합친 표는 어느 서비스도 책임지지 않는다.
 *
