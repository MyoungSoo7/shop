import { describe, it, expect } from 'vitest';
import { FALLBACK_MENUS, resolveFallbackMenus, type FallbackMenuNode } from '@/data/menuFallback';

/**
 * 네비게이션 회귀 고정 — 트리 기반 네비게이션이 <b>메뉴 시드와 같은 것</b>을 그리는지.
 *
 * <p>정본은 order-service 의 메뉴 시드 마이그레이션이고 이 폴백은 그 사본이다. 둘의 **경로**
 * 정합은 `menu-route-gate.test.mjs` 가 CI 에서 대조하고, 이 파일은 그보다 한 겹 위 —
 * **역할별로 무엇이 보이는가**를 고정한다. 서버 필터링(역할+권한)과 폴백 필터링(역할만)이
 * 같은 결과를 내는 것이 전제이므로, 여기가 깨지면 그 전제가 흔들린 것이다.
 */

const labelsOf = (role: string) => resolveFallbackMenus(role).map((m) => m.label);
const childrenOf = (role: string, groupName: string) =>
  resolveFallbackMenus(role).find((m) => m.name === groupName)?.children.map((c) => c.name) ?? [];

describe('상단 네비 — 역할별 항목·순서 고정', () => {
  // '반품·교환'은 '승인' 바로 뒤다 — '승인' 의 자식이 아니라 형제. '승인' 은 ITEM 이라
  // 자식을 붙이려면 GROUP 으로 바꿔야 하고, 그러면 지금 그 링크로 들어가는 취소·환불 승인
  // 큐가 링크가 아니게 된다. 시드(V20260827160000)도 같은 자리에 끼우도록 뒤를 한 칸 민다.
  // '문의 응대'도 같은 이유로 '승인'의 형제다 — 자리는 '반품·교환' 바로 뒤(시드
  // V20260828100000 도 같은 자리에 끼우도록 뒤를 한 칸 민다).
  it('ADMIN: 대시보드·상품관리·배송·승인·반품교환·문의응대·시스템', () => {
    expect(labelsOf('ADMIN')).toEqual([
      '대시보드', '상품관리', '배송', '승인', '반품·교환', '문의 응대', '시스템',
    ]);
  });

  it('MANAGER: ADMIN 목록에서 시스템만 빠진다', () => {
    // 반품 응대는 CS 업무라 MANAGER 에게도 열린다 — 서버의 /admin/return-requests/**
    // 매처가 ADMIN,MANAGER 이므로 죽은 링크가 아니다. 문의 응대도 같다
    // (/admin/inquiries/** 매처가 ADMIN,MANAGER).
    expect(labelsOf('MANAGER')).toEqual([
      '대시보드', '상품관리', '배송', '승인', '반품·교환', '문의 응대',
    ]);
  });

  it('USER: 구매자 화면 12개 + 파트너 콘솔 + 셀러 콘솔', () => {
    // 잔액 화면은 결제 직전에 "얼마까지 낼 수 있나"를 확인하러 오는 경로다.
    // 대량주문은 관리자 기능이 아니라 구매자가 자기 주문을 올리는 경로다 — SHOP 최상위.
    // 나눠 결제는 주문(20)과 잔액 확인(30) 사이 — 주문에서 결제로 이어지는 순서다.
    // 내 알림(35)은 잔액(30) 다음 — 둘 다 "내 것"을 보는 개인 화면이라 붙여 둔다.
    // 내 문의(40)는 그 뒤 — 화면 경로가 /inquiries 가 아니라 /my/inquiries 인 것은,
    // nginx 두 벌이 inquiries 세그먼트를 게이트웨이로 프록시하기 때문이다.
    // 여러 곳 배송(45)은 맨 뒤 — 앞 항목들의 sort_order 가 5 칸씩 차 있어 중간에 끼우려면
    // 시드의 뒤를 전부 밀어야 한다. 새 항목이라 뒤에 붙여도 잃는 것이 없다.
    // 배송지 주소록(50)도 같은 이유로 그 뒤다.
    // 포인트 선물(55)도 마찬가지 — '내 포인트·상품권'(30) 옆이 어울리지만 그 자리에 끼우려면
    // 뒤를 전부 밀어야 하고, 시드를 미는 UPDATE 는 이미 배포된 sort_order 를 건드린다.
    // 카테고리 탐색(60)도 같은 이유로 맨 뒤다. 자리는 '주문하기' 앞이 어울린다 — 둘러보고
    // 고르는 순서라서 — 하지만 앞자리를 만들려면 이미 배포된 sort_order 를 전부 밀어야 한다.
    // 이벤트(65)도 같은 이유로 맨 뒤다 — 화면은 이 SPA 에 있지만 부르는 API 는
    // marketing-service 것이다(ADR 0045).
    //
    // 파트너(70)는 CORP 영역의 첫 줄이고, 이 목록에서 유일하게 *모두에게 보이지만 모두가 쓸
    // 수는 없는* 항목이다. 이 저장소의 메뉴 역할 어휘에 PARTNER 가 없어서 그렇다 — 없는 역할을
    // 적으면 서버는 그것을 모르니 아무도 막히지 않는데 원장에는 통제가 있는 것처럼 남는다.
    // 그래서 로그인까지만 걸고, 입점사가 아닌 계정이 눌렀을 때 partner-service 가 403
    // NOT_A_PARTNER 로 돌려주면 화면이 "이 계정은 입점 조직에 속해 있지 않습니다" 를 그린다.
    // 즉 여기 이 한 줄이 늘어난 것은 노출이 늘었다는 뜻이지 권한이 늘었다는 뜻이 아니다.
    //
    // 셀러(75)도 같은 성질의 줄이다 — 영역만 SELLER 로 다르다(파는 쪽과 사는 기업을 한
    // 영역에 넣지 않는다). 역할 어휘에 SELLER 가 없는 것도 같고, 차단이 서버 한 곳
    // (403 NOT_A_SELLER_MEMBER / 422 NOT_A_SELLER_ORG)인 것도 같다.
    expect(labelsOf('USER')).toEqual([
      '주문하기', '추천받기', '대량주문', '나눠 결제', '내 포인트·상품권', '내 알림', '내 문의',
      '여러 곳 배송', '배송지 주소록', '포인트 선물', '카테고리 탐색', '이벤트', '파트너', '셀러',
    ]);
  });

  it('파트너 콘솔 (USER): 매출 대시보드·주문 내역', () => {
    expect(childrenOf('USER', '파트너 콘솔')).toEqual(['매출 대시보드', '주문 내역']);
  });

  /**
   * 셀러 콘솔에는 심사가 없다. 승인·반려는 운영자 화면이고 '시스템 관리' 아래에 있다 —
   * 여기 넣으면 그룹 roles 가 USER+ADMIN 이 되어야 하는데, 폴백의 accessible 은 정확 일치라
   * 운영자에게 자기가 403 을 받는 링크(/seller/products)가 함께 그려진다.
   */
  it('셀러 콘솔 (USER): 상품 등록·주문 출고 — 심사는 여기 없다', () => {
    expect(childrenOf('USER', '셀러 콘솔')).toEqual(['상품 등록', '주문 · 출고']);
    expect(childrenOf('USER', '셀러 콘솔')).not.toContain('상품 심사');
  });

  /**
   * 운영자 역할에는 파트너 콘솔이 뜨지 않는다. 막으려는 것이 아니라 <b>다른 화면</b>이라서다 —
   * 운영자는 전사 매출을 /admin/system/sales-stats 에서 본다. 여기에 같이 띄우면 두 화면이
   * 같은 질문에 다른 숫자를 답하는 것처럼 보인다(하나는 내 조직, 하나는 전사).
   */
  it('ADMIN·MANAGER 에게는 파트너 콘솔이 뜨지 않는다', () => {
    expect(labelsOf('ADMIN')).not.toContain('파트너');
    expect(labelsOf('MANAGER')).not.toContain('파트너');
  });

  /**
   * 셀러 콘솔도 같다. 운영자는 파는 쪽 화면이 아니라 심사 화면을 쓴다 — 같이 띄우면
   * 자기가 403 을 받는 링크가 네비에 하나 생길 뿐이다.
   */
  it('ADMIN·MANAGER 에게는 셀러 콘솔이 뜨지 않는다', () => {
    expect(labelsOf('ADMIN')).not.toContain('셀러');
    expect(labelsOf('MANAGER')).not.toContain('셀러');
  });

  it('미인증: 아무 것도 없다', () => {
    expect(labelsOf('')).toEqual([]);
    expect(resolveFallbackMenus(null)).toEqual([]);
  });
});

describe('사이드바 — 그룹별 항목', () => {
  it('배송 (ADMIN): 배송 관리·배송비 정책', () => {
    expect(childrenOf('ADMIN', '배송')).toEqual(['배송 관리', '배송비 정책']);
  });

  it('배송 (MANAGER): 배송비 정책은 빠진다 — 서버가 ADMIN 으로 막는 금액 결정 경로', () => {
    // 고객이 실제로 지불하는 금액을 바꾸는 표면이다. MANAGER 에게 열면 눌러도 되돌려보내지는
    // 죽은 링크가 된다 — 서버가 막는 것을 화면이 먼저 감춘다.
    expect(childrenOf('MANAGER', '배송')).toEqual(['배송 관리']);
  });

  it('시스템 (ADMIN): 32개 항목', () => {
    // 카탈로그 3종이 붙어 있다 — 무엇으로 묶이나(분류) → 언제 앞에 세우나(진열) → 무엇으로 고르나(옵션)
    // 그 다음이 관제(운영관리), 그리고 콘텐츠 관리 2종(게시판·교육).
    expect(childrenOf('ADMIN', '시스템 관리')).toEqual([
      '메뉴 관리', '공통코드 관리', 'RBAC 관리', '이커머스 카테고리', '진열 편성', '옵션 카탈로그', '운영관리',
      '게시판 관리', '교육 관리',
      // 내부잔액 원장 2종의 운영 콘솔 — 수기 지급·발행은 없던 재산을 만들고 소멸은 지운다.
      '포인트 운영', '기프트카드 운영',
      // 운영 콘솔 4종 — 감사 로그는 적재만 하고 아무도 못 보던 표면, 회원 관리는 승인 API 는 있는데
      // 대상을 못 찾던 표면, 리뷰·쿠폰은 내리고 멈추는 방법이 DB 직접 수정뿐이던 표면.
      // 조직·멤버십은 회원 관리 바로 뒤 — 개인(회원)을 본 다음 그 사람이 속한 조직으로 이어진다.
      '감사 로그', '회원 관리', '조직 · 멤버십', '리뷰 관리', '쿠폰 운영',
      // 마지막 둘은 사라진 '정산운영' 그룹에서 옮겨 왔다 — 부르는 API 가 order-service 것이라
      // 이 저장소에서도 살아 있다.
      '환불 운영', '셀러 등급',
      // dentis 관리자 콘솔에서 옮겨 온 4종. 앞의 셋은 "보기만 하는" 표면이고, 작업 큐만
      // MANAGER 에게도 열린다 — 밀린 주문을 실제로 처리하는 쪽이라서다.
      '권한 계정', '지표 추이', '판매 통계', '작업 큐',
      // 수강 신청은 '교육 관리'(과정·차시) 와 짝이지만 자리는 맨 뒤다 — 그룹의 sort_order 가
      // 빈틈없이 차 있어 중간에 끼우면 그 자리의 항목과 겹친다.
      '수강 신청', '강사 관리',
      // 팝업 관리는 교육이 아니라 사이트 콘텐츠지만 자리는 역시 맨 뒤다 — 이유는 같다.
      '팝업 관리',
      // 댓글 관리는 '게시판 관리'(게시판 정의) 와 다른 표면이다 — 이미 달린 댓글을 게시판·글을
      // 건너뛰고 훑는 유일한 자리. 자리는 역시 맨 뒤.
      '댓글 관리',
      // 동의 이력도 맨 뒤 — 감사 로그 옆에 두고 싶지만 그 자리의 sort_order 가 차 있다.
      // 읽기 전용이라 고치는 버튼이 없고, 그래서 '운영'이 아니라 '이력'이라 부른다.
      '동의 이력',
      // 상품 옵션은 위의 '옵션 카탈로그' 바로 옆이 어울리지만 자리는 맨 뒤다 — 이유는 같다.
      // 이름이 비슷해도 다른 표면이다: 카탈로그는 축·값 사전, 이쪽은 상품별 실물 SKU.
      '상품 옵션',
      // 이벤트 프로모션은 부르는 API 가 이 저장소의 다른 화면들과 달리 marketing-service 것이다
      // (ADR 0045). 메뉴 원장은 order-service 의 menus 한 벌뿐이라 행은 여기 있다.
      '이벤트 프로모션',
      // 상품 심사는 셀러가 올린 신청서를 운영자가 승인·반려하는 자리다. 부르는 API 는
      // seller-service 것이고(이 그룹에서 두 번째로 order-service 밖의 API), 셀러 콘솔이
      // 아니라 여기 있는 이유는 대상이 "내 조직"이 아니라 전체 신청서여서다.
      '상품 심사',
      // 배치 실행 원장·주문 상태 이력도 맨 뒤 — 이유는 위와 같다(그룹의 sort_order 가 차 있다).
      // 둘 다 "지금까지 답할 방법이 DB 직접 조회뿐이던" 질문을 화면으로 옮긴 자리다:
      // "어제 그 배치가 돌았나" 와 "이 주문 왜 이 상태인가".
      // 주문 상태 이력만 MANAGER 도 열 수 있지만(실제로 여는 쪽이 CS 다) 부모 그룹이
      // ADMIN 전용이라 MANAGER 의 트리에는 통째로 안 뜬다 — 아래 그룹 테스트가 그걸 본다.
      '배치 실행 원장', '주문 상태 이력',
    ]);
  });

  it('시스템 그룹은 ADMIN 전용이라 MANAGER 에게는 통째로 없다', () => {
    expect(childrenOf('MANAGER', '시스템 관리')).toEqual([]);
  });
});

describe('사이드바 머리글', () => {
  it('시스템은 상단 네비 라벨과 사이드바 제목이 다르다 (시스템 / 시스템 관리)', () => {
    const system = resolveFallbackMenus('ADMIN').find((m) => m.name === '시스템 관리');

    expect(system?.label).toBe('시스템');
    expect(system?.name).toBe('시스템 관리');
    expect(system?.description).toBe('System Administration');
  });
});

describe('폴백 id 유일성', () => {
  // 역할로 거르기 *전*의 원본을 본다. 거른 트리만 보면 역할이 달라 한 번도 같이 그려지지 않는
  // 두 항목의 충돌을 놓친다 — 실제로 겹쳤던 최상위 '반품·교환'(ADMIN·MANAGER)과 시스템 자식
  // '수강 신청'(ADMIN)이 그런 짝이었다.
  const flatten = (nodes: FallbackMenuNode[]): FallbackMenuNode[] =>
    nodes.flatMap((node) => [node, ...flatten(node.children ?? [])]);

  it('트리 전체에서 id 가 겹치지 않는다', () => {
    const all = flatten(FALLBACK_MENUS);
    const seen = new Map<number, string>();
    const dupes: string[] = [];

    all.forEach((node) => {
      const owner = seen.get(node.id);
      // 어느 둘이 겹쳤는지까지 남긴다 — 개수만 세면 고칠 자리를 다시 찾아야 한다.
      if (owner !== undefined) dupes.push(`${node.id}: ${owner} ↔ ${node.name}`);
      else seen.set(node.id, node.name);
    });

    expect(dupes).toEqual([]);
    expect(all.length).toBe(seen.size);
  });

  it('id 는 전부 음수다 — 서버가 주는 실제 메뉴 id 와 섞이면 안 된다', () => {
    expect(flatten(FALLBACK_MENUS).filter((node) => node.id >= 0)).toEqual([]);
  });
});
